...
* 在备份 ONTAP 系统上设置的超级用户和用户 ID ，用于使用 Astra 控制中心备份和还原应用程序。在ONTAP 命令行中运行以下命令：
...
* Astra Trident `volumesnapshotclass` 由管理员定义的对象。请参见 Astra Trident https://docs.netapp.com/us-en/trident/trident-use/vol-snapshots.html["说明"^] 使用 Astra Trident 启用和测试卷快照。
